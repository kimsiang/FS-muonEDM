...
#include "musrRadialField.hh"             // Radial E field and Uniform B field implemented by Kim Siang Khaw
#include "musrGradientField.hh"             // Gradient B field implemented by Kim Siang Khaw
...
G4VPhysicalVolume* musrDetectorConstruction::Construct()  {
  ...
  while (!feof(fSteeringFile)) {
	...
	if (strcmp(typeOfField,"fromfile")==0) {
    ...
    else if (strcmp(typeOfField,"radial")==0) {
	  double fieldValue[6];
	  char logicalVolumeName[100];
	  double positionX, positionY, positionZ;
	  G4double half_x = pp1;  // halfwith of the box with the field
	  ...
	  G4LogicalVolume* logVol = FindLogicalVolume(logicalVolumeName);
	  if (logVol==NULL) {
	    sprintf(eMessage,"musrDetectorConstruction.cc::Construct(): GLOBAL FIELD (radial): Logical volume \"%s\" not found.", logicalVolumeName);
	    musrErrorMessage::GetInstance()->musrError(FATAL,eMessage,false);
	  }
	  G4double fieldValue_tmp[6] = {
	  fieldValue[0]*CLHEP::tesla,        fieldValue[1]*CLHEP::tesla,        fieldValue[2]*CLHEP::tesla,
	  ...
	  musrRadialField* myElementRadialField = new musrRadialField(fieldValue_tmp, half_x, half_y, half_z, logVol, position);
	  myElementRadialField->SetElementFieldName(tmpString2);
	}

    else if (strcmp(typeOfField,"gradient")==0) {
	  double fieldValue[6];
	  char logicalVolumeName[100];
	  double positionX, positionY, positionZ;
	  G4double half_x = pp1;  // halfwith of the box with the field
	  ...
	  G4LogicalVolume* logVol = FindLogicalVolume(logicalVolumeName);
	  if (logVol==NULL) {
	    sprintf(eMessage,"musrDetectorConstruction.cc::Construct(): GLOBAL FIELD (gradient): Logical volume \"%s\" not found.", logicalVolumeName);
	    musrErrorMessage::GetInstance()->musrError(FATAL,eMessage,false);
	  }
	  G4double fieldValue_tmp[6] = {
	  fieldValue[0]*CLHEP::tesla,        fieldValue[1]*CLHEP::tesla,        fieldValue[2]*CLHEP::tesla,
	  ...
	  musrGradientField* myElementGradientField = new musrGradientField(fieldValue_tmp, half_x, half_y, half_z, logVol, position);
	  myElementGradientField->SetElementFieldName(tmpString2);
	}
