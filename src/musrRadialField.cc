...
musrRadialField::musrRadialField(G4double EMF[6], G4double half_X, G4double half_Y, G4double half_Z, G4LogicalVolume* lv, G4ThreeVector c)
...
void musrRadialField::addFieldValue(const G4double point[4],
...
G4double musrRadialField::GetNominalFieldValue() {
...
void musrRadialField::SetNominalFieldValue(G4double newFieldValue){
...
G4bool musrRadialField::isOutside(G4ThreeVector& local) const
{
  return (std::fabs(local.z()) > fieldLength/2.0 || std::fabs(local.x()) > fieldWidth/2.0 || std::fabs(local.y()) > fieldHeight/2.0);
}

G4bool musrRadialField::isWithin(G4ThreeVector& local) const
{
  return (std::fabs(local.z()) < fieldLength/2.0 && std::fabs(local.x()) < fieldWidth/2.0 && std::fabs(local.y()) < fieldHeight/2.0);
}
